...
/*
 * function createMonthSelect(){ var selectmonth=newElement('select');
 * selectmonth.name="aboluo-selectmonth"; selectmonth.id="aboluo-selectmonth";
 * selectmonth.onchange=function(e){ var aclick=withClass("aboluo-aclick");
 * createTabledate(withID("aboluo-yearSelect").value,selectmonth.options[selectmonth.selectedIndex].value);
 * if(aclick==""){ //说明没选,或选的当天,算出选的这个月有多少天,与原来的那个月的天数一对比,如果原来的天数大于现在的天数,那么对换
 * //这里先算当前月当前天,然后算出选择的那个月总天数,然后对比,如果当前天大于选择的那个月那天,对换 var
 * pervdays1=getCurrMonthLashDay(withID("aboluo-yearSelect").value,selectmonth.options[selectmonth.selectedIndex].value);
 ...
 * }else{
 * setRigth(withID("aboluo-yearSelect").value,selectmonth.options[selectmonth.selectedIndex].value,new
 * Date().getDate()); } }else{ var adate=aclick.getAttribute("date"); var
 * aarr=adate.split("-"); aarr[0]=parseInt(aarr[0]); aarr[1]=parseInt(aarr[1]);
 * aarr[2]=parseInt(aarr[2]); var
 * pervdays=getCurrMonthLashDay(withID("aboluo-yearSelect").value,selectmonth.options[selectmonth.selectedIndex].value);
 * if(aarr[2]>pervdays){ aarr[2]=pervdays; }
 * setRigth(withID("aboluo-yearSelect").value,selectmonth.options[selectmonth.selectedIndex].value,aarr[2]); } };
 * //选中当前月份 var Nowtime=new Date(); var currMonth=Nowtime.getMonth(); for(var
 * i=0;i<12;i++){ selectmonth.options.add(new Option((i+1)+"月",i+1));
 * if(currMonth==i){ selectmonth.options[i].selected=true; } } var
 * next=withClass("aboluo-month-a-next"); var parent=next.parentNode;
...
// 初始化当前日期的选中样式
function setCurrentDayStyle() {
	var rows = withClass("aboluo-rilitbody").rows;
	// 循环日历表格中的每一个单元格 设置当前日期样式
...
function setRigth(year, yue, day) {
	...
	// TODO 需要改成判断是否为休息日
	var bool = false;
	var dateString = formatByYearyueday(year, yue, day);
	for (var n = 0; n < jjrmodeltimelist.length; n++) {
		if (jjrmodeltimelist[n].indexOf(dateString) != -1)
			bool = true;
	}
	// if(bool){
	// withClass("aboluo-ssjjr")
	// }
	// if(bool){
	...
	var szrqbutton = newElement('a');
	szrqbutton.className = "easyui-linkbutton m-t-10 mw-70 m-r-10 z-easyui-bg-primary l-btn l-btn-small"; // 设置class
	szrqbutton.style.marginLeft = "30px";

	var buttonSpan = newElement('span');
	buttonSpan.className = "l-btn-left";
	buttonSpan.style.align = "center";
	var spanText = newElement('span');
	spanText.className = "l-btn-text";
	if (bool) {
		spanText.innerText = "设置为交易日";
		// 设置为交易日时，填参数delete
		szrqbutton.setAttribute("onclick", "javascript:aboluoSetrq('delete');");

	} else {
		spanText.innerText = "设置为休息日";
		// 设置为交易日时，填参数add
		szrqbutton.setAttribute("onclick", "javascript:aboluoSetrq('add');");

	}

	buttonSpan.appendChild(spanText);
	szrqbutton.appendChild(buttonSpan);
...
function setaclass(year, yue, day) {
	...
	if (year1 == year && yue == month1 + 1 && day1 == day) {
		...
		for (var i = 0; i < arr.length; i++) {
			var date = arr[i].getAttribute("date");
//			var formatDate = formatByDate(date);
			var datearr = date.split("-");
...
function leftrightclick() {
	...
	righta.onclick = function() {
		...
		if (monthvalue == 12) {
			...
			// TODO 根据当前年，当前月加载数据
			getjjrszModelByYearAndMonth(yearvalue, monthvalue);
			monthvalue = 1;
		} else {
			monthvalue += 1;
		}
	...
	today.onclick = function() {
		...
		getjjrszModelByYearAndMonth(date.getFullYear(), monthselect.value);
		createTabledate(yearselect.value, monthselect.value);
...
function aboluoSetrq(type) {
	...
	var dateArr = date.split("-");
	if ("add" == type) {
		jjrmodeltimelist.push(formatByDate(date));
		addHolidayTimeList.push(formatByDate(date));
		...
		setRigth(dateArr[0], dateArr[1], dateArr[2]);
	} else if ("delete" == type) {
		var delIndex;
		for(var n=0;n<jjrmodeltimelist.length;n++){
			if(jjrmodeltimelist[n].indexOf(formatByDate(date))!=-1)
				delIndex=n;
		}
		jjrmodeltimelist.splice(delIndex,1);
		deleteHolidayTimeList.push(formatByDate(date))
		createTabledate(parseInt(withID("aboluo-yearSelect").value),
				parseInt(withID("aboluo-selectmonth").value));
		setaclass(dateArr[0], dateArr[1], dateArr[2]);
		setRigth(dateArr[0], dateArr[1], dateArr[2]);
	 ...
	 * date=getAclickDomDate(); var datearr=date.split("-"); //重新加载当前所属年月的节假日
	 * getjjrszModelByYearAndMonth(datearr[0],datearr[1]);
	 * 
	 * createTabledate(datearr[0],datearr[1]); //创建对应的table(日期)
	 * setRigth(datearr[0],datearr[1],datearr[2]); //设置右边明细栏内容 }else{
	 * alert("设置失败"); } }, error:function(json){ alert("设置失败"); } });
	 */
}

//返回这一年的所有周末日期的数组
function getWeekendByYear(y){
	var time = new Date();
	...
	for(var m = 1;m <= 12;m++){
		...
		for(var i=1;i<=tempTime.getDate();i++){
	        ...
	        var dateString = formatByYearyueday(y, m, date);
	        if(day == 6 || day == 0){
	        	weekends.push(dateString);
	        }
	    }
	}
    return weekends;
